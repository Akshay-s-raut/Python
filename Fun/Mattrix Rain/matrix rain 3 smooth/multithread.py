...
class RainDrop:
    ...
    def toString(self):
        return self.color+"{}".format(self.character)
...
buffer = []
def makeBuffer(Width,Length):
    frame=[]
    while(True):
        frame2 = makeFrame(Width,Length,frame)
...
def printBuffer():
    os.system('cls')
    for i in buffer:
        print(getFrameAsAString(i))
        os.system('cls')


t1 = threading.Thread(target=makeBuffer,args=(Width,Length,))
t2 = threading.Thread(target=printBuffer)
