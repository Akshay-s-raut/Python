...
def updateSandbox(window):
    global particles,rate,radius,gravity
    ...
    for i in particles:
        ...
        i.Velocity[1] = i.Velocity[1] + gravity*rate
...
def displayTotalMomentum():
    ...
    for i in particles:
        ...
        vavg = vavg + getMagnitude(i.Velocity)
    vavg = vavg/N
    print("Total Momentum at {} = {}".format(round(time.time()-start,2),(round(px,2),round(py,2))))
    print("Average Velocity at {} = {}".format(round(time.time()-start,2),round(vavg,2)))
    print(particles[0].Velocity[1])

def displayTotalEnergy():
    global particles,start,winndow_size
    ke = 0
    pe = 0
    for i in particles:
        ke = ke + 0.5*(i.Mass)*(getMagnitude(i.Velocity)**2)
        pe = pe + (i.Mass)*(gravity)*(winndow_size[1]-i.Position[1])
    print("Kinetic Energy at {} = {}".format(round(time.time()-start,2),round(ke,2)))
    print("Potential Energy at {} = {}".format(round(time.time()-start,2),round(pe,2)))
    print("Total Energy at {} = {}".format(round(time.time()-start,2),round(pe+ke,2)))
# AddParticles(N)
particles.append(Particle([250,200],[0,0],5,(255,0,0)))
